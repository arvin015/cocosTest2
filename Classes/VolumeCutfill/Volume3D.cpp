...
//namespace VolumeCutfill {
    ...
    void Volume3D::setData(CC3DLayer* cc3DLayer, const vector<VertexInfo> &vecs) {
    ...
    void Volume3D::loadEdges() {
        ...
        for (int i = 0; i < cntside; i++) {
            spEdges.push_back(SAFE_RETAIN(makeEdgeSprite(vs[i].p3d, vs[(i+1)%cntside].p3d, volume3d)));
            spEdges.push_back(SAFE_RETAIN(makeEdgeSprite(vs[i].p3d, vs[i].p3d + height, volume3d)));
            spEdges.push_back(SAFE_RETAIN(makeEdgeSprite(vs[i].p3d + height, vs[(i+1)%cntside].p3d + height, volume3d)));
        }
    ...
    bool Volume3D::getMinDisPoint(const Vec2 &point, const Vec2 &startPoint, float maxDis, VertexInfo &rs, float &dist) {
        int index = -1;
        dist = 1000;
    ...
    bool Volume3D::checkCanCut(const Vec2 &p1, const Vec2 &p2) {
    ...
    void Volume3D::cutPolygon(const Vec2 &p1, const Vec2 &p2, vector<VertexInfo> &leftVecList, vector<VertexInfo> &rightVecList) {
        bool isStart = false;
        bool isStop = false;
    ...
    void Volume3D::playTransAnim(float duration, const Vec2 &p1, const Vec2 &p2, const TRANS_DIRECTION &direction) {
        Vec3 offset;
